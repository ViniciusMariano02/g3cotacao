...
export const Products = () => {
    ...
    useEffect(() => {
        fetch("http://8b38091fc43d.sn.mynetname.net:2000/cotacao/fornecedor/529")
            .then((res) => res.json())
            .then(
                (result) => {
                    setIsLoaded(true);
        ...
        <div>
            ...
            <div className="product">
                ...
                <div className="select">
                        <select
                            onChange={(q) => {
                                setFilterParam(q.target.value);
                            }}
                            className="custom-select"
                            aria-label="Filtro teste"
                        >
                            <option>{product.id} {product.descricao}</option>
                            <option>{product.id} {product.descricao}</option>
                            <option>{product.id} {product.descricao}</option>
                            <option>{product.id} {product.descricao}</option>
                            <option>{product.id} {product.descricao}</option>
                                        ...
                                        products.map((products) => (  
                                            products.detalhe.map((detalhes, i) => {
                                                return (
                                                        ...
                                                        <td className= "placeholder01" onDoubleClick={handleEdit}>{}
                                                            {isEdit ? 
                                                            (
                                                            <input 
                                                                className= "placeholder01"
